# 👋 Hey, I'm **Jorgo**

I'm an **AI Product Manager** with a CS background, building tech products across the **US and Europe**.  

Currently pursuing my **Master's in Engineering Management & Leadership at Rice University** (Fulbright Scholar), with a focus in **Computer & Data Science** — bridging **product strategy**, **AI development**, and **intuitive design**.

I believe great products come from listening deeply to customers, experimenting boldly, and turning complex, scattered ideas into clarity.

---

### Education

**Master of Engineering Management & Leadership** – *Computer & Data Science Concentration*  
* Rice University · Houston, TX · 4.0 GPA · *Aug 2025 – Dec 2026* 

**Bachelor of Arts in Computer Science** – *Minors in Mathematics & Information Systems*  
* American University in Bulgaria · *Aug 2016 – Dec 2020*

---

### Experience

I've worked at the intersection of **technology, AI, and product**, leading cross-functional teams from 0→1 MVPs to scaled platforms across fintech, real estate, SaaS, and wellness.

| Role | Organization | Industry |
|------|--------------|----------|
| **Teaching Assistant – Product Engineering Management** | Rice University | *AI Tools · Product Discovery · Miro* |
| **Technical Product Creator** | Redocs | *NYC Real Estate Compliance (AI/SaaS)* |
| **Technical Product Manager** | EasyPay Albania | *FinTech & Open Banking* |
| **Head of CX & Product Success Manager** | Publer | *Global Social Media Management SaaS* |
| **Product Manager** | Trainly | *Habit-Forming Wellness App (MVP)* |

---

### Projects

**[ReHub Portal](https://www.jorgoqirjaj.com/projects)**  
AI-powered compliance platform serving 40K+ NYC buildings — built using Claude AI to streamline real estate regulatory workflows.

**[EasyPay Open Banking](https://easypay.al/en/openbanking/)**  
Led the development of Albania's first Open Banking integration — real-time account aggregation and direct bank payments for a 20+ person engineering team.

**[Waze Churn Prediction Model](https://waze-pm.streamlit.app/)**  
Machine learning model and dashboard predicting user churn patterns using behavioral and engagement data.

**[Trainly](https://www.trainlyapp.com/)**  
A habit-forming fitness app designed to help users build consistency through small, sustainable changes — validated through 20+ user interviews.

---

### Skills

**Product:**  
AI Product Management · 0→1 MVP Development · Product Discovery & Validation · Roadmapping · A/B Testing · User Research · Agile & Scrum · Data 

**Tech:**  
Python · SQL · Machine Learning · Data Visualization · EDA · AI Prototyping & Integration  · Claude Code  · Claude Design · ML 

**Tools:**  
Miro · Jira · Figma · Notion · Azure Boards · Zapier · Airtable · HubSpot

---

### Languages

English (Fluent) · Albanian (Native) · Spanish · Bulgarian · Greek (Intermediate)

---

### Connect with Me

📎 [**Portfolio Website**](https://www.jorgoqirjaj.com)  
💼 [**LinkedIn**](https://www.linkedin.com/in/jorgoqirjaj/)  
🐙 [**GitHub**](https://github.com/jorgo-qirjaj)  
📧 [**Email**](mailto:qirjaj.jorgo@gmail.com)  
📄 [**CV**](https://www.jorgoqirjaj.com/)
